...
RAW_PATH = "/media/chi/比赛盘/hack_data" #数据路径，要保证下一级文件夹是1.训练集和2.测试集

# NUM_LABEL = "./label_map/num_label.txt"
# NUM_LABEL_R = "./label_map/num_label_r.txt"

TRAIN_PATH = os.path.join(RAW_PATH, "1.训练集")
TRAIN_DATA = os.path.join(TRAIN_PATH, "hack_lip_train")
TRAIN_LABEL = os.path.join(TRAIN_PATH, "hack_lip_train.txt")
VAL_DATA = ''

TEST_PATH = os.path.join(RAW_PATH, "2.测试集")
TEST_DATA = os.path.join(TEST_PATH, "hack_lip_test")
TEST_SUB = "./result/sub_2.csv" #提交的文件
num_class = 1000  #类的数量，根据需要改

def file_deal(root_path, list_file = TRAIN_LABEL):
    ...
    video_list = list(filter(lambda x: len(os.listdir(os.path.join(root_path, x))) >= 5 and len(os.listdir(os.path.join(root_path, x))) <= 24,
                                  sig_names))#有效的文件名列表
